...
@HiltViewModel
class AbsenceTraineesViewModel @Inject constructor(
    ...
    fun getAbsence()=viewModelScope.launch {

        _state.emit(UIState.Loading)
        try {
            val campId = getCurrentCamp()?.id?.toInt()
            val response = connect2.connect().getAbsenceTrainees(campId!!,"Bearer $token")
